...
with app.app_context():
    ...
    major = Major.query.filter_by(name="数据科学与大数据技术", college_id=college.id).first()
    if not major:
        print("ERROR: 找不到数据科学与大数据技术专业")
        exit()
    ...
    # 专业方向模块
    print("=== 任选四门（专业方向）模块 ===")
    major_dir = Module.query.filter_by(name="任选四门（专业方向）", major_id=major.id).first()
    if major_dir:
        print(f"模块ID: {major_dir.id}, 名称: {major_dir.name}")
        courses = Course.query.filter_by(module_id=major_dir.id).all()
        print(f"课程数量: {len(courses)}")
        for c in courses:
            print(f"  - {c.course_code} {c.name} ({c.credit}学分)")
    else:
